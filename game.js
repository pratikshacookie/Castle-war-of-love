...
let B=null;
const WORLD={w:680,h:500,r:14,playerSpeed:185,kingSpeed:72,shotSpeed:410};
const WALLS=[
...
function boss(){
 const maxHp=175+S.score*13;
 B={running:true,win:false,keys:{},shots:[],particles:[],last:performance.now(),kingTimer:0,player:{x:55,y:70,hp:maxHp,maxHp,dir:'right',shootCd:0,shield:false,inv:0},king:{x:610,y:350,hp:190+S.score*4,maxHp:190+S.score*4,dir:'left',shootCd:4.2,flash:0,dead:false,phase:1},cleanup:null};
 page(`<p class="small">THE FINAL TRIAL</p><h2>⚔️ WOLFY VS THE MONSYETER KING</h2><div class="battleHud"><div><b>🤴 WOLFY · KNIGHT</b><div class="bar"><i id="php"></i></div><span id="pht"></span></div><div><b>👺 MONSYETER KING</b><div class="bar enemy"><i id="khp"></i></div><span id="kht"></span></div></div><div id="arena" class="mazeArena"><canvas id="battleCanvas"></canvas><div class="bossWarning" id="bossWarning">⚠️ HIDE BEHIND THE WALLS</div><div class="battleText" id="battleText">Move through the castle. Aim toward the King and tap SHOOT.</div><div class="deathOverlay" id="death"></div></div><div class="battleControls"><div class="dpad"><button data-dir="up">▲</button><div><button data-dir="left">◀</button><button data-dir="down">▼</button><button data-dir="right">▶</button></div></div><div class="combatBtns"><button class="combat attackBtn" id="attackBtn">✨ SHOOT</button><button class="combat shieldBtn" id="shieldBtn">🛡️ SHIELD</button></div></div><p class="hint">Phone: hold a direction to move. Tap SHOOT to fire where Wolfy is facing. Hold SHIELD. Desktop: WASD/arrows · Space · Shift.</p>`);
 initBattle();
}
...
function shoot(){if(!B?.running||B.player.shootCd>0)return;const p=B.player,k=B.king;let dx,dy;if(clearLine(p,k)){dx=k.x-p.x;dy=k.y-p.y}else{if(p.dir==='left')dx=-1,dy=0;else if(p.dir==='right')dx=1,dy=0;else if(p.dir==='up')dx=0,dy=-1;else dx=0,dy=1}const l=Math.hypot(dx,dy)||1;B.shots.push({x:p.x,y:p.y,vx:dx/l*WORLD.shotSpeed,vy:dy/l*WORLD.shotSpeed,owner:'p',life:0});p.shootCd=.32;burst(p.x,p.y,'#d9efff',5);msg(clearLine(p,k)?'✨ MAGIC SHOT — HIT THE KING!':'✨ MAGIC SHOT — GET AROUND THE WALL!')}
function kingShoot(){const p=B.player,k=B.king;if(!clearLine(k,p)){msg('🧱 The wall protects Wolfy.');return}const dx=p.x-k.x,dy=p.y-k.y,l=Math.hypot(dx,dy)||1;B.shots.push({x:k.x,y:k.y,vx:dx/l*WORLD.shotSpeed*.82,vy:dy/l*WORLD.shotSpeed*.82,owner:'k',life:0});k.shootCd=4.5;const w=document.querySelector('#bossWarning');if(w){w.textContent='⚠️ KING ATTACKS — HIDE!';w.classList.add('danger');setTimeout(()=>w.classList.remove('danger'),800)}msg('👺 THE KING THROWS HIS MAGIC WEAPON!')}
function update(dt){const p=B.player,k=B.king;if(!B.running)return;let dx=0,dy=0;if(B.keys.w||B.keys.arrowup||B.keys.up)dy--;if(B.keys.s||B.keys.arrowdown||B.keys.down)dy++;if(B.keys.a||B.keys.arrowleft||B.keys.left)dx--;if(B.keys.d||B.keys.arrowright||B.keys.right)dx++;if(dx||dy){if(Math.abs(dx)>Math.abs(dy))p.dir=dx>0?'right':'left';else p.dir=dy>0?'down':'up'}moveCircle(p,dx,dy,WORLD.playerSpeed,dt,WORLD.r);p.shield=!!B.keys.shield;p.shootCd=Math.max(0,p.shootCd-dt);p.inv=Math.max(0,p.inv-dt);k.flash=Math.max(0,k.flash-dt);if(!k.dead){if(k.hp<k.maxHp*.35)k.phase=2;let kdx=0,kdy=0;const d=Math.hypot(p.x-k.x,p.y-k.y);if(d>185){kdx=(p.x-k.x)/(d||1);kdy=(p.y-k.y)/(d||1)}else if(d<125){kdx=(k.x-p.x)/(d||1);kdy=(k.y-p.y)/(d||1)}moveCircle(k,kdx,kdy,k.phase===2?WORLD.kingSpeed*1.28:WORLD.kingSpeed,dt,WORLD.r+3);if(Math.abs(kdx)>Math.abs(kdy))k.dir=kdx>0?'right':'left';else if(kdy)k.dir=kdy>0?'down':'up';k.shootCd=Math.max(0,k.shootCd-dt);if(k.shootCd<=0&&clearLine(k,p))kingShoot()}
 for(let i=B.shots.length-1;i>=0;i--){const s=B.shots[i];const ox=s.x,oy=s.y;s.x+=s.vx*dt;s.y+=s.vy*dt;s.life+=dt;if(s.x<0||s.x>WORLD.w||s.y<0||s.y>WORLD.h||WALLS.some(w=>segmentRect(ox,oy,s.x,s.y,w))){B.shots.splice(i,1);continue}const target=s.owner==='p'?k:p;if(!target.dead&&Math.hypot(s.x-target.x,s.y-target.y)<WORLD.r+8){B.shots.splice(i,1);if(s.owner==='p'){k.hp=Math.max(0,k.hp-22);k.flash=.15;burst(k.x,k.y,'#ffffff',12);msg(`💥 DIRECT HIT! KING HP −22`);if(k.hp<=0){winBattle();return}}else if(p.inv<=0){if(p.shield){p.inv=.2;burst(p.x,p.y,'#9ed5ff',10);msg('🛡️ BLOCKED!')}else{p.hp=Math.max(0,p.hp-(k.phase===2?20:17));p.inv=.65;burst(p.x,p.y,'#ff6878',10);msg(`💥 WOLFY HIT! ${k.phase===2?'THE KING IS ENRAGED — ':' '}HIDE OR SHIELD!`);if(p.hp<=0){loseBattle();return}}}}}
...
function updateHud(){if(!B)return;const a=document.querySelector('#php'),b=document.querySelector('#khp');if(a)a.style.width=`${B.player.hp/B.player.maxHp*100}%`;if(b)b.style.width=`${B.king.hp/B.king.maxHp*100}%`;const c=document.querySelector('#pht'),d=document.querySelector('#kht');if(c)c.textContent=`${Math.ceil(B.player.hp)}/${B.player.maxHp} HP`;if(d)d.textContent=`${Math.ceil(B.king.hp)}/${B.king.maxHp} HP`}
